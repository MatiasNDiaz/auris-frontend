 ...
 *
 * Los índices 0–3 forman la fila visible: la del frente y tres asomando
 * detrás, cada una un poco más chica y más tenue. La card que deja de estar activa
...
const desktopFan: Slot[] = [
  { x: 0, scale: 1, opacity: 1, zIndex: 40 },
  { x: 124, scale: 0.96, opacity: 1, zIndex: 30 },
  { x: 248, scale: 0.92, opacity: 0.95, zIndex: 20 },
  { x: 372, scale: 0.88, opacity: 0.7, zIndex: 10 },
];
...
const mobileFan: Slot[] = [
  { x: 0, scale: 1, opacity: 1, zIndex: 40 },
  { x: 82, scale: 0.95, opacity: 1, zIndex: 30 },
  { x: 156, scale: 0.9, opacity: 0.7, zIndex: 20 },
];
        ...
        {services.map((service, index) => {
          ...
          return (
            <motion.article
              ...
              aria-hidden={!isFront}
              className={cn(
                // `origin-top`: las de atrás se dibujan más chicas para dar
                // profundidad, y achicándose desde el centro sus topes
                // quedaban escalonados. Anclando la escala arriba, las cuatro
                // comparten el borde superior y la fila se lee alineada.
                "absolute top-0 left-0 h-40 w-30 origin-top overflow-hidden rounded-2xl border border-cream-50/25 shadow-xl sm:h-46 md:h-50 md:w-38",
